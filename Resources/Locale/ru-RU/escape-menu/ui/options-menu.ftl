...
ui-options-admin-enable-overlay-playtime = Отображать наигранное время
ui-options-admin-enable-overlay-starting-job = Показывать начальную должность
ui-options-admin-overlay-merge-distance = Дистанция слияния стака информации
ui-options-admin-overlay-ghost-fade-distance = Дистанция затемнения оверлея призрака от курсора
